...
export interface LiveProjectFlowPhase {
...
/**
 * Programmatic triggers for the Film Demo Director. Deliberately excludes
 * any way to confirm/approve the source scope: that action must always
 * come from a genuine click on the real button rendered below, never from
 * driver code — see `confirmScope` in this file, which stays a plain
 * internal function with no imperative-handle entry.
 */
export interface LiveProjectFlowHandle {
  runCreateVault(): Promise<void>;
  runLoadProject(): Promise<void>;
  runStartScoping(): Promise<void>;
  runGenerateReport(): Promise<void>;
  getPhase(): LiveProjectFlowPhase;
}

interface LiveProjectFlowProps {
  readonly onBack: () => void;
  /** Present only in Film Demo Director mode: prefilled values and a
   * synthetic artifact root that skips the folder picker. Absent (the
   * default) leaves the manual flow entirely unchanged. */
  readonly demoConfig?: FilmDemoConfig;
  /** Masks full local paths in this view's own text (the Director's
   * separate narration panel never receives them either way). */
  readonly filmDemoMode?: boolean;
  readonly onPhaseChange?: (phase: LiveProjectFlowPhase) => void;
}
...
export const LiveProjectFlow = forwardRef<LiveProjectFlowHandle, LiveProjectFlowProps>(
  function LiveProjectFlow({ onBack, demoConfig, filmDemoMode = false, onPhaseChange }, ref) {
    const [step, setStep] = useState<FlowStep>("setup");
    const [vaultPath, setVaultPath] = useState("");
    const [password, setPassword] = useState(demoConfig?.password ?? "");
    const [ownerName, setOwnerName] = useState(demoConfig?.ownerName ?? "Owner");
    ...
    const [question, setQuestion] = useState(demoConfig?.question ?? "What is the current project state?");
    const [analysis, setAnalysis] = useState<AnalysisData | null>(null);

    const runtimeAvailable = isTauriRuntime();

    useEffect(() => {
      onPhaseChange?.({ step, busy, error });
    ...
    async function createVault(): Promise<void> {
      const path = demoConfig ? demoConfig.vaultPath : await selectNewVaultPath();
      if (!path) return;
      await run(async () => {
    ...
    async function pickAndLoadProject(): Promise<void> {
      const root = demoConfig ? demoConfig.artifactRoot : await selectArtifactRoot();
      if (!root) return;
      await run(async () => {
    ...
    // This is the one action never exposed through `LiveProjectFlowHandle`:
    // it only ever runs from the real button's own onClick below, so no
    // driver (including the Film Demo Director) can approve on its behalf.
    ...
    useImperativeHandle(
      ref,
      () => ({
        runCreateVault: createVault,
        runLoadProject: pickAndLoadProject,
        runStartScoping: startScoping,
        runGenerateReport: generateReport,
        getPhase: () => ({ step, busy, error }),
      }),
      // eslint-disable-next-line react-hooks/exhaustive-deps
      [step, busy, error, password, ownerName, question, vaultPath, demoConfig],
    );
      ...
      <section className="view active" aria-label="Live project">
        <div className="page-shell workspace-shell live-project-shell">
          ...
          {step === "reported" && analysis ? (
            <section className="finding-rail-section live-project-panel" id="live-project-report">
              ...
              {reportError ? (
                ...
                <>
                  ...
                  <div className="status-list" id="live-continuity-break">
                  ...
                  <section className="finding-rail-section" id="live-recommended-action">
                  ...
                  <div id="live-evidence-inspector">
