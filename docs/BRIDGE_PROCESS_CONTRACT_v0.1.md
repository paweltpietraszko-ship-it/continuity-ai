...
- Success data: `{"session_id": "<string>"}`
...
- Effect: clears decrypted attestation evidence and the in-memory analysis/snapshot/question from Bridge state. The encrypted vault file and its retained analyses are untouched.

### `load_project`

- Required: `artifact_root` (string; path to the artifact directory only, not the vault)
- Success data: `{"artifact_evidence_count": <int>, "evidence_count": <int>}`
- Failure categories: `validation_error` (missing artifact root, malformed evidence manifest, checksum mismatch between a manifest entry and the file on disk, or any other ingestion failure)
- A failed `load_project` leaves all prior Bridge state (artifact records, composed records, spans, retained analysis, snapshot, question) exactly as it was before the call.
...
- Success data: `evidence_id`, `evidence_count`, `citation_cards`, and current analysis fields (the analysis is recomputed against the newly confirmed evidence)
...
- After restart and `unlock_vault`, a valid retained analysis (if one was successfully persisted before the restart) is restored from the encrypted vault. The parent must not send `analyze_project` merely to display restored history.
...
- `invalid` — the newest retained entry in the vault failed canonical restoration validation. There is deliberately no fallback to an older valid entry: an invalid newest entry never causes stale history to be silently presented as current.

When `retained_analysis_status` is `invalid`, `has_analysis` is `false` and none of the analysis fields or `citation_cards` are present in the response. Vault authentication and the neutral `evidence_count`/`artifact_evidence_count` fields remain available regardless.
...
semantic validation of analysis results (initial and restored)
...
displaying backend-returned fields as-is
```

The UI must not infer evidence ownership, semantic roles, source status, continuity-break kind, quotations, evidence/span IDs, or next action; it only displays what the Bridge response already contains. This document does not prescribe visual design or Rust implementation details.
